  ...
  for(int i=1;i<5;i++){
    for(int j=0;j<6;j++){
      for(int k=1;k<5;k++){
	    ...
	    likelihood+=log(1.0*(N_ss[i][j][k][l]+N_os[i][j][k][l])*(par[i+5*j]*(1-par[k+5*l])+par[k+5*l]*(1-par[i+5*j])))*N_ss[i][j][k][l] - 1.0*(N_ss[i][j][k][l]+N_os[i][j][k][l])*(par[i+5*j]*(1-par[k+5*l])+par[k+5*l]*(1-par[i+5*j]));}
...
// input file
  ...
  std::string outputName=path+"misID."+sample+".root";
...
//  TH1F* h1_dummy = new TH1F("h1_dummy","QmisID vs el eta",8,etaBins);
  ...
  h1_pT2->SetLineColor(2);h1_pT2->SetMarkerColor(2);h1_pT2->SetMarkerStyle(20);
  ...
  for(int i=1;i<7;i++){
    h1_pT1->SetBinContent(i,p[5*i-5]);h1_pT1->SetBinError(i,e[5*i-5]);
    h1_pT2->SetBinContent(i,p[5*i-4]);h1_pT2->SetBinError(i,e[5*i-4]);
    h1_pT3->SetBinContent(i,p[5*i-3]);h1_pT3->SetBinError(i,e[5*i-3]);
    h1_pT4->SetBinContent(i,p[5*i-2]);h1_pT4->SetBinError(i,e[5*i-2]);
    h1_pT5->SetBinContent(i,p[5*i-1]);h1_pT5->SetBinError(i,e[5*i-1]);
  }

    for(int i=1;i<5;i++){
    h2_eta1->SetBinContent(i,p[i-1]);h2_eta1->SetBinError(i,e[i-1]);
    h2_eta2->SetBinContent(i,p[5+i-1]);h2_eta2->SetBinError(i,e[5+i-1]);
    h2_eta3->SetBinContent(i,p[10+i-1]);h2_eta3->SetBinError(i,e[10+i-1]);
    h2_eta4->SetBinContent(i,p[15+i-1]);h2_eta4->SetBinError(i,e[15+i-1]);
    h2_eta5->SetBinContent(i,p[20+i-1]);h2_eta5->SetBinError(i,e[20+i-1]);
    h2_eta6->SetBinContent(i,p[25+i-1]);h2_eta6->SetBinError(i,e[25+i-1]);
  }
...
//nominal
  ...
  lg->AddEntry(h1_pT1,"pT [15,28] GeV");
  lg->AddEntry(h1_pT2,"pT [28,60] GeV");
