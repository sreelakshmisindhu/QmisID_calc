...
void MisIDrate2D(std::string sample) {
...
//  TH2F* h_SS = (TH2F*)f0->Get("h_SS");
  ...
  ifstream os;
// input file
  ss.open(("ss."+sample+".txt").c_str());
  os.open(("os."+sample+".txt").c_str());

  std::string legend_title="data15 likelihood";
  std::string outputName="out."+sample+".root";
  ...
  double bmin[24]={0};
  double bmax[24]={0};
  for(int i=0;i<24;i++){vstrt[i]=0.001;stp[i]=0.0001;bmin[i]=0.;bmax[i]=1.;}
...
//  minuit.FixParameter(0);
  ...
  TH1F* h1_dummy = new TH1F("h1_dummy","QmisID vs el eta",6,etaBins);
//  TH1F* h1_dummy = new TH1F("h1_dummy","QmisID vs el eta",8,etaBins);
  ...
  h1_pT1->SetLineColor(2);h1_pT1->SetMarkerColor(2);
  h1_pT2->SetLineColor(4);h1_pT2->SetMarkerColor(4);
  h1_pT3->SetLineColor(6);h1_pT3->SetMarkerColor(6);
  h1_pT4->SetLineColor(7);h1_pT4->SetMarkerColor(7);
...
//nominal
  TLegend* lg = new TLegend(0.65,0.2,0.85,0.5,legend_title.c_str());
  lg->AddEntry(h1_pT1,"pT [28,60] GeV");
