    ...
    for j in range (1,5):
        for i in range(1,7):
            ...
            if Type == "fit": 
                ...
                hist.SetBinContent(i,j, abs(sigma4_bw[j-1+4*(i-1)] - sigma2_bw[j-1+4*(i-1)])/sigma3_bw[j-1+4*(i-1)])
            elif Type == "total":
                fcn_unc = abs(sigma3[j-1+4*(i-1)] - sigma3_bw[j-1+4*(i-1)])/sigma3_bw[j-1+4*(i-1)]
                width_unc = abs(sigma4_bw[j-1+4*(i-1)] - sigma2_bw[j-1+4*(i-1)])/sigma3_bw[j-1+4*(i-1)]
                fit_unc = s3_err_bw[j-1+4*(i-1)]/sigma3_bw[j-1+4*(i-1)]
                closure_unc = closure_hist.GetBinContent(i,j)
                total = np.sqrt(width_unc*width_unc+ fcn_unc*fcn_unc +fit_unc*fit_unc+ closure_unc*closure_unc)
